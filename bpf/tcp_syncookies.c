/**
 *	Tempesta xFW TCP SYN cookies module
 *
 * SPDX-FileCopyrightText: © 2026 Tempesta Technologies, Inc.
 * SPDX-License-Identifier: GPL-2.0-or-later
 */
#define BPF_PROGRAM
#define XFW_TCP_SYNCOOKIES
 ...
 */
static __always_inline int
xfw_tcp_syncookies_syn_filter(XfwGlobalCtx *ctx, struct tcphdr *th)
{
 ...
 */
static __always_inline int
xfw_tcp_syncookies_ack_filter(const XfwGlobalCtx *ctx, struct tcphdr *th)
{
...
xfw_xdp_filter_chain(XfwGlobalCtx *ctx)
{
	...
	if (th->syn) {
		CHAIN(xfw_tcp_syncookies_syn_filter, ctx, th);
	} else if (th->ack) {
		CHAIN(xfw_tcp_syncookies_ack_filter, ctx, th);
		/*
...
SEC("xdp")
int xfw_tcp_syncookies(struct xdp_md *xdp)
{
	XfwGlobalCtx ctx;
