...
/* Infinite timeout - entry will be blocked until LRU eviction. */
#define XFW_SYN_DROP_INFINITE_TIMEOUT UINT64_MAX
			 ...
			 uint64_t hash, uint64_t now)
{
	    ...
	    || now > entry->jtxtstamp + entry->max_delay)
	{
		 ...
		 */
		entry->blocked_until = XFW_SYN_DROP_INFINITE_TIMEOUT;
		return XFW_MAKE_CTX_DROP(ctx, XFW_DROP_SYN_BLOCKED);
	}
