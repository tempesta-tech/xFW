...
class LocalVeth(BaseNetwork):
    ...
    async def create_links(self, link_from: str, link_to: str):
        ...
        assert code == 0 or 'File exists' in stderr, \
            f'Could not create veth peer: {stderr}'
    ...
    async def _create_namespace(self, name: str):
        ...
        assert code == 0 or 'File exists' in stderr, \
            f'Could not create namespace: {stderr}'
    ...
    async def _move_iface_into_namespace(self, iface: str, namespace: str):
        ...
        assert code == 0, \
            f'Could not move clients iface into namespace: {stderr}'

    async def _turn_on_iface(self, name: str, namespace: str = None):
        code, *_ = await run_cmd(
            cmd=self._with_netns(f"ip link set {name} up", namespace),
            logger=self.logger
        )
    ...
    async def _turn_off_offload_on_iface(self, iface: str):
        ...
        assert code == 0, \
            f'Could not turn off rx tx offloading {_}'
    ...
    async def prepare(self):
        await self.create_links(
            link_from=self.config.client_interface,
            link_to=self.config.backend_interface
        )
        await self._create_namespace(
            name=self.config.client_namespace
        )
        ...
        await self._turn_on_iface(
            name=self.config.client_interface,
            namespace=self.config.client_namespace
        )
        await self._turn_on_iface(
            name=self.config.backend_interface
        )
        await self._turn_off_offload_on_iface(
...
class LocalGateVeth(LocalVeth):
    ...
    async def _nft_table_prepare(
        ...
        nft_config = "\n".join([
            ...
            '        iifname "{client_iface}" oifname "{backend_iface}" counter accept;'.format(client_iface=client_iface, backend_iface=backend_iface),
            '        iifname "{backend_iface}" oifname "{client_iface}" counter accept;'.format(client_iface=client_iface, backend_iface=backend_iface),
            "   }",
            "   chain prerouting {",
    ...
    async def prepare(self):
        # create link
        await self.create_links(
            link_from=self.config.client_interface_host,
            link_to=self.config.client_interface
        )
        await self.create_links(
            link_from=self.config.backend_interface_host,
            link_to=self.config.backend_interface
        )
        ...
        await self._create_namespace(
            name=self.config.backend_namespace
        )
...
class LocalNatVeth(LocalGateVeth):
    ...
    async def prepare(self):
        ...
        await self._toggle_ip_forwarding()
        await self._force_ipv6_neighbor_update(
            ipv6=self.config.backend_ipv6_host,
            namespace=self.config.backend_namespace
        )
...
class LocalVirtualizedNIC(LocalVeth):
    ...
    async def _turn_off_iface(self, iface: str, namespace: str = None):
        ...
        assert code == 0, \
            f'Could not turn off iface={iface}: {stderr}'
