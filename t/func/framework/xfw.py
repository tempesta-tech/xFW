...
class XFW(NetworkStateful):
    ...
    async def wait_for_iface_ready(self):
        code, _, stderr = await run_cmd(
            cmd=f"ip -br a show {self.server_iface}", logger=self.logger
        )
    ...
    async def wait_for_daemon_started_once(self, cleanup_on_loaded_programs: bool = False):
        code, _, stderr = await run_cmd(
            cmd=f"{self.path_to_executable} --start", logger=self.logger, log_output=True
        )
    ...
    async def wait_for_daemon_stop(self):
        code, _, stderr = await run_cmd(
            cmd=f"{self.path_to_executable} --stop", logger=self.logger, log_output=True
        )
    ...
    async def wait_for_grpc_connection_ready(self, ip: str, port: int):
        try:
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(host=ip, port=port), timeout=self.timeout
            )
            writer.close()
    ...
    async def restart_daemon(self):
        code, _, stderr = await run_cmd(
            cmd=f"{self.path_to_executable} --restart", logger=self.logger, log_output=True
        )
    ...
    async def set_mtu(self, size: int = 1500) -> None:
        ...
        for iface in ifaces:
            code, _, stderr = await self.run_host_cmd(cmd=f"ip link set {iface} mtu {size}")
