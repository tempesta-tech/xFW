...
from tests.metrics.utils import DnsRequests, InvalidEthTypeRawClient
...
async def test_invalid_l4(
    ...
    await clickhouse_client.connect()
    await ether_raw_server.start()
    await invalid_l2_ip4_raw_client.start()

    src_mac, dst_mac = await asyncio.gather(
        invalid_l2_ip4_raw_client.get_mac_address(), ether_raw_server.get_mac_address()
    )
...
async def test_blocked_by_tcp_syncookies_rule_invalid_syn_cookie_34_bit(
    xfw_with_forced_syncookie,
    tcp_server,
    tcp_raw_client,
    ...
    tcp_raw_client.auto_ack_seq = False

    async with metric_analyzer.track_clickhouse_metric(
        clickhouse_client, ip_to_search=tcp_raw_client.ip_clickhouse
    ) as metric:
        await tcp_raw_client.send_packet(get_tcp_packet(flag="S", seq=random.randrange(1, 9999999)))

        syn_ack = await tcp_raw_client.receive_packet()
        assert syn_ack is not None, "xfw did not reply with SYN-ACK"
        assert tcp_raw_client.has_flag(
            syn_ack, "SA"
        ), f"Unexpected reply flags {syn_ack.flags}; expected SA"

        await tcp_raw_client.send_packet(get_tcp_packet(flag="A"))
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("invalid_l2_ip4_raw_client", "send_dns_ip4_large_response_packet", id="ip4"),
        pytest.param("invalid_l2_ip6_raw_client", "send_dns_ip6_large_response_packet", id="ip6"),
    ],
    indirect=["invalid_l2_raw_client"],
)
async def test_blocked_by_dns_anomaly_udp_response_packet_is_too_large_43_bit(
    invalid_l2_raw_client: InvalidEthTypeRawClient,
    send_method: str,
    clickhouse_client,
    metric_analyzer,
    ether_raw_server,
    ...
    assert metric.blocked_by_dns_anomaly_dns_udp_response_packet_is_too_large
