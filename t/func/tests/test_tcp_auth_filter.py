...
import pytest
from framework.asyn import TcpRawServer, TcpRawClient, TcpClient, TcpServer
...
async def test_block_closed_session(
    ...
    assert await asyncio.gather(
        tcp_raw_client.send_data("test_data_1"),
        tcp_raw_server.receive_data()
...
async def test_unblock_non_existing_session(
    ...
    response = await tcp_raw_server.receive_packet()
    assert response == 'A', \
        f'TCP packet with A without session is not allowed'
...
async def test_allow_tcp_flood_after_connection(
    ...
    await tcp_raw_client.send(TCP(flags=tcp_flag))
    assert await tcp_raw_server.receive_packet() == tcp_flag, \
