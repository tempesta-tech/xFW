...
async def download_ddos_archive(config: ConfigSettings, conf_logger):
    if os.path.exists(config.xfw_ddos_examples_temp_dir):
        conf_logger.info(
            'Skipped downloading ddos examples. '
            'File already exists'
        )
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            'pkt.UDP.null.pcapng', '10.10.10.10', 500, 98, False,
            'xfw {{}}',
        ...
        pytest.param(
            'pkt.UDP.fragmented.pcap', '10.10.10.10', 500, 98, False,
            'xfw {{}}',
            id='udp-fragmented'
        ),
        ...
        pytest.param(
            'amp.UDP.bacnet.IOT.37810.pcapng', '10.10.10.1', 500, 98, False,
            """
            xfw {{ 
        ...
        pytest.param(
            'amp.UDP.DNSANY.pcap', '10.10.10.10', 500, 98, False,
            'xfw {{ dns_filter; }}',
            id='udp-dns-any',
        ),
        ...
        pytest.param(
            'amp.UDP.IOT.port37810.JSON.pcap', '10.10.10.10', 500, 60, True,
            '''
            xfw {{ 
                ratelimit=iot pps=10 bps=10000;
                dst ip4.udp : ratelimit=iot {{
        ...
        pytest.param(
            'amp.UDP.isakmp.pcap', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
                src ip4.udp : block {{
                    :4500
        ...
        pytest.param(
            'amp.UDP.manyprotocols.pcapng', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
                defaults {{ dst: block; }}
        ...
        # is the same.
        pytest.param(
            'amp.UDP.memcached.ntp.cldap.pcap', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
        ...
        pytest.param(
            'amp.UDP.snmp.src161.pcapng', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
        ...
        pytest.param(
            'amp.UDP.UBNT.src10001.pcapng', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
                src ip4.udp : block {{
                    :10001
        ...
        pytest.param(
            'pkt.UDP.rdm.fixedlength.pcapng', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
        ...
        pytest.param(
            'pkt.ICMP.largeempty.pcap', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
                defaults {{ icmp ip4: block; }}
            }}
        ...
        pytest.param(
            'pkt.IPV4.randomprotofield.pcap', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ }}
            ''',
            id='ip4-random-proto',
        ...
        pytest.param(
            'amp.TCP.reflection.SYNACK.pcap', '10.10.10.10', 500, 95, True,
            '''
            xfw {{ 
                tcp_auth_filter;
            }}
        ...
        pytest.param(
            'amp.TCP.syn.optionallyACK.optionallysamePort.pcapng', '10.10.10.10', 500, 95, True,
            '''
            xfw {{ 
                tcp_auth_filter;
        ...
        pytest.param(
            'pkt.TCP.DOMINATE.syn.ecn.cwr.pcapng', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
                tcp_anomaly_filter;
                tcp_auth_filter;
        ...
        pytest.param(
            'pkt.TCP.synflood.spoofed.pcap', '10.10.10.10', 500, 98, True,
            '''
            xfw {{ 
                tcp_anomaly_filter;
            }}
        ...
        pytest.param(
            'amp.dns.RRSIG.fragmented.pcap', '10.10.10.10', 500, 28, True,
            '''
            xfw {{ 
                dns_filter;
            }}
...
async def test_execute_pcap(
    ...
    await xfw.rules_set(xfw_rule.format(
        host=ether_raw_server.ip_testing,
        port=ether_raw_server.port
    ))

    received_packets_task = asyncio.create_task(count_packets(
