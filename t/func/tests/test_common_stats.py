...
from scapy.layers.inet import Ether, IP, TCP, UDP, ICMP, IP_PROTOS
from scapy.layers.l2 import ARP
from scapy.layers.inet6 import IPv6, IPv6ExtHdrFragment
from scapy.all import Raw, ETH_P_IP, ETH_P_IPV6
...
class SendInvalidPacketsMixin(EtherRawClient):
    async def send_eth_eapol_packet(self, src_mac: str, dst_mac: str):
        packet = Ether(dst=dst_mac, src=src_mac, type=socket.ETHERTYPE_VLAN) / Raw(load=b'some_data')
    ...
    async def send_bad_ip4_header_greater(self, src_mac: str, dst_mac: str):
        packet = Ether(dst=dst_mac, src=src_mac, type=ETH_P_IP)
        packet = packet / (bytes(IP(version=4, ihl=16, src=self.ipv4, dst=self.remote_ip)) + b'invalid_part')
        await self.loop.sock_sendall(self.socket, bytes(packet))
        self.logger.info(f'Sending L2 packet {packet}')
    ...
    async def send_bad_ip6_fragmented(self, src_mac: str, dst_mac: str):
        ...
        packet = packet / IPv6(
            src=self.ipv6,
            dst=self.remote_ip,
        ) / IPv6ExtHdrFragment(id=12345) / Raw(b'flag 10')
        await self.loop.sock_sendall(self.socket, bytes(packet))
        self.logger.info(f'Sending L2 packet {packet}')
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            'send_eth_eapol_packet', ETH_P_IP,
        ...
        pytest.param(
            dict(
                xfw_l2_unknown_ingress_packets=[10, 15],
                xfw_l2_unknown_ingress_bytes=[230, 1000],
            ),'send_eth_custom_packet',  ETH_P_IP,
            id='eth-l2-bad-protocol-custom',
        ),
        pytest.param(
            dict(
                xfw_eth_badhdr_ingress_packets=10,
                xfw_eth_badhdr_ingress_bytes=540
            ), 'send_bad_eth_header', ETH_P_IP,
            id='eth-send-bad-header',
            marks=pytest.mark.skip('OS prevent sending packages less then 14 bytes'),
        ),
        pytest.param(
            dict(
                xfw_ip4_badhdr_ingress_packets=10,
                xfw_ip4_badhdr_ingress_bytes=240
            ),
            'send_bad_ip4_header_less', ETH_P_IP,
            id='ip4-tcp-header-less',
        ),
        pytest.param(
            dict(
                xfw_ip4_badhdr_ingress_packets=10,
                xfw_ip4_badhdr_ingress_bytes=460
            ),
            'send_bad_ip4_header_greater', ETH_P_IP,
            id='ip4-tcp-header-greater',
        ),
        pytest.param(
            dict(
                xfw_ip4_badhdr_ingress_packets=10,
                xfw_ip4_badhdr_ingress_bytes=340
            ),
            'send_bad_ip4_bad_ip_version', ETH_P_IP,
            id='ip4-bad-ip-version',
        ),
        pytest.param(
            dict(
                xfw_ip4_fragmented_ingress_packets=10,
                xfw_ip4_fragmented_ingress_bytes=410
            ),
            'send_bad_ip4_fragmented', ETH_P_IP,
            id='ip4-block-fragmented',
        ),
        pytest.param(
            dict(
                xfw_ip6_badhdr_ingress_packets=10,
                xfw_ip6_badhdr_ingress_bytes=490
            ),
            'send_bad_ip6_header_less', ETH_P_IPV6,
            id='ip6-header-less',
        ),
        pytest.param(
            dict(
                xfw_ip6_badhdr_ingress_packets=10,
                xfw_ip6_badhdr_ingress_bytes=540
            ),
            'send_bad_ip6_bad_ip_version', ETH_P_IPV6,
            id='ip6-bad-ip-version',
        ),
        pytest.param(
            dict(
                xfw_ip6_fragmented_ingress_packets=10,
                xfw_ip6_fragmented_ingress_bytes=690
            ),
            'send_bad_ip6_fragmented', ETH_P_IPV6,
            id='ip6-block-fragmented',
        ),
        pytest.param(
            dict(
                xfw_tcp_badhdr_ingress_packets=10,
                xfw_tcp_badhdr_ingress_bytes=490
            ),
            'send_bad_tcp_headers', ETH_P_IP,
            id='tcp-bad-header-len',
        ),
        pytest.param(
            dict(
                xfw_udp_badhdr_ingress_packets=10,
                xfw_udp_badhdr_ingress_bytes=400
            ),
            'send_bad_udp_headers', ETH_P_IP,
            id='udp-bad-header-len',
        ),
        pytest.param(
            dict(
                xfw_icmp_badhdr_ingress_packets=10,
                xfw_icmp_badhdr_ingress_bytes=540
            ),
            'send_bad_icmp_headers', ETH_P_IP,
            id='icmp-bad-header-len',
            marks=pytest.mark.skip('ISSUE: 332')
        ),
        pytest.param(
            dict(
                xfw_arp_ingress_packets=10,
                xfw_arp_ingress_bytes=540
            ),
            'send_bad_arp_headers', ETH_P_IP,
            id='arp-bad-header',
            marks=pytest.mark.skip('ISSUE: 332')
        ),
        pytest.param(
            dict(
                xfw_l4_unsupported_ingress_packets=10,
                xfw_l4_unsupported_ingress_bytes=460
            ),
            'send_l4_unsupported_ip_proto', ETH_P_IP,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            dict(
                xfw_preload_ingress_packets=10,
                xfw_preload_ingress_bytes=540
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            dict(
                xfw_l2_unknown_egress_packets=[10, 15],
                xfw_l2_unknown_egress_bytes=[540, 800]
            ),
            'send_eth_eapol_packet', ETH_P_IP,
            id='eth-l2-bad-protocol-eapol',
            marks=pytest.mark.skip('ISSUE: 332')
        ...
        pytest.param(
            dict(
                xfw_l2_unknown_egress_packets=[10, 12],
                xfw_l2_unknown_egress_bytes=[540, 800]
            ),'send_eth_custom_packet',  ETH_P_IP,
            id='eth-l2-bad-protocol-custom',
            marks=pytest.mark.skip('ISSUE: 332')
        ...
        pytest.param(
            dict(
                xfw_eth_badhdr_egress_packets=10,
                xfw_eth_badhdr_egress_bytes=540
            ), 'send_bad_eth_header', ETH_P_IP,
            id='eth-send-bad-header',
            marks=pytest.mark.skip('ISSUE: 332')
        ),
        pytest.param(
            dict(
                xfw_ip4_badhdr_egress_packets=10,
                xfw_ip4_badhdr_egress_bytes=540
            ),
            'send_bad_ip4_header_less', ETH_P_IP,
            id='ip4-tcp-header-less',
            marks=pytest.mark.skip('ISSUE: 332')
        ...
        pytest.param(
            dict(
                xfw_ip6_badhdr_egress_packets=10,
                xfw_ip6_badhdr_egress_bytes=540
            ),
            'send_bad_ip6_header_less', ETH_P_IPV6,
            id='ip6-header-less',
            marks=pytest.mark.skip('ISSUE: 332')
        ...
        pytest.param(
            dict(
                xfw_tcp_badhdr_egress_packets=10,
                xfw_tcp_badhdr_egress_bytes=540
            ),
            'send_bad_tcp_headers', ETH_P_IP,
            id='tcp-bad-header-len',
            marks=pytest.mark.skip('ISSUE: 332')
        ...
        pytest.param(
            dict(
                xfw_udp_badhdr_egress_packets=10,
                xfw_udp_badhdr_egress_bytes=540
            ),
            'send_bad_udp_headers', ETH_P_IP,
            id='udp-bad-header-len',
            marks=pytest.mark.skip('ISSUE: 332')
        ...
        pytest.param(
            dict(
                xfw_l4_unsupported_egress_packets=10,
                xfw_l4_unsupported_egress_bytes=540
            ),
            'send_l4_unsupported_ip_proto', ETH_P_IP,
            id='ip4-block-unsupported-sctp',
            marks=pytest.mark.skip('ISSUE: 332')
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            dict(
                xfw_preload_egress_packets=10,
                xfw_preload_egress_bytes=540
            ),
...
async def test_egress_preload_stats(
    ...
    async with xfw.metrics_diff(stats_counters) as diff:
        await asyncio.gather(*[
            udp_ip4_server.send('message') for _ in range(10)
        ])
...
async def test_clickhouse_reports_stats(
    ...
    stats_counters = [
        'xfw_not_reported_incident_packets',
        'xfw_not_reported_incident_bytes'
    ]
    await udp_ip4_server.start()
    await udp_ip4_client.start()
...
async def check_clean_counters(
    ...
    async with xfw.metrics_diff(stats_counters) as diff:
        await asyncio.gather(*[
            client.send_bad_udp_headers(src_mac, dst_mac) for _ in range(10)
        ])
