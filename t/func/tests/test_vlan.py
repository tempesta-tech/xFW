...
from scapy.layers.inet import Ether, ETH_P_IP, IP, UDP
...
class Ether802Dot1QServer(EtherRawServer):
    ...
    async def receive_message(self, message: bytes) -> tuple[Optional[Ether], Optional[tpacket_auxdata]]:
        start_at = time.time()
        packet = ""
        ...
        if packet and message not in packet:
            self.logger.info(f"The expected L2 package is missing from the received ones - {packet}")
...
async def ether_802_dot_1q_server(ether_raw_server: EtherRawServer):
    server = server_cloner(
        ether_raw_server,
        fabric=Ether802Dot1QServer,
        amount=1
    )[0]
    yield server
    await server.stop()
...
async def test_send_multiple_vlan(
    ...
    src_mac, dst_mac = await gather(
        ...
        ether_802_dot_1q_server.get_mac_address()
    )
    packet = (
