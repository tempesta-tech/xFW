...
from framework.utils import compare_metrics_diff, client_cloner, get_tcp_packet
from framework.asyn import (
    TcpIpV4RawClient,
    TcpIpV6RawClient,
    TcpV4Server,
    TcpV6Server, TcpRawClient
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            get_tcp_packet(flag='SF'), 'ip4',
            dict(
                xfw_tcp_anom_bad_flags_packets=10,
                xfw_tcp_anom_bad_flags_bytes=620
            ),
            id='ip4-bad-flag-u',
        ),
        pytest.param(
            get_tcp_packet(flag='SF'), 'ip6',
            dict(
                xfw_tcp_anom_bad_flags_packets=10,
                xfw_tcp_anom_bad_flags_bytes=820
            ),
            id='ip6-bad-flag-u',
        ),
        pytest.param(
            get_tcp_packet(flag='S', seq=0), 'ip4',
            dict(
                xfw_tcp_anom_syn_bad_seq_packets=10,
                xfw_tcp_anom_syn_bad_seq_bytes=620
            ),
            id='ip4-bad-seq-0',
        ),
        pytest.param(
            get_tcp_packet(flag='S', seq=0), 'ip6',
            dict(
                xfw_tcp_anom_syn_bad_seq_packets=10,
                xfw_tcp_anom_syn_bad_seq_bytes=820
            ),
            id='ip6-bad-seq-0',
        ),
        pytest.param(
            get_tcp_packet(flag='S',options=[]), 'ip4',
            dict(
                xfw_tcp_anom_syn_no_options_packets=10,
                xfw_tcp_anom_syn_no_options_bytes=540
            ),
            id='ip4-syn-no-options'
        ),
        pytest.param(
            get_tcp_packet(flag='S', options=[]), 'ip6',
            dict(
                xfw_tcp_anom_syn_no_options_packets=10,
                xfw_tcp_anom_syn_no_options_bytes=740
            ),
            id='ip6-syn-no-options'
        ),
        pytest.param(
            get_tcp_packet(flag='S', payload=b'1'), 'ip4',
            dict(
                xfw_tcp_anom_syn_has_data_packets=10,
                xfw_tcp_anom_syn_has_data_bytes=630
            ),
            id='ip4-syn-payload'
        ),
        pytest.param(
            get_tcp_packet(flag='S', payload=b'1'), 'ip6',
            dict(
                xfw_tcp_anom_syn_has_data_packets=10,
                xfw_tcp_anom_syn_has_data_bytes=830
            ),
            id='ip6-syn-payload'
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            'ip4',
            dict(
                xfw_tcp_anom_zero_port_packets=10,
                xfw_tcp_anom_zero_port_bytes=740
            ),
            id='ip4-zero-port',
        ),
        pytest.param(
            ...
            dict(
                xfw_tcp_anom_zero_port_packets=10,
                xfw_tcp_anom_zero_port_bytes=940
            ),
            id='ip6-zero-port',
        ),
    ]
...
async def test_zero_port(
    ...
    async with xfw.metrics_diff(stats_counters, wait_softirq=True) as diff:
        await asyncio.gather(*[
            client.send(client.valid_syn_packet)
            for i in range(5)
        ] + [
            client_2.send(client_2.valid_syn_packet)
            for i in range(5)
        ])
