...
from framework.utils import metrics_increased, client_cloner
...
async def test_tcp_auth_filter_tcp_flood_from_non_existing_session(
    ...
    assert await tcp_raw_server.receive_packet() == 'A', \
...
async def test_tcp_flags_filter(
    ...
    async with xfw.metrics_diff(metrics) as diff_metrics:
        await asyncio.gather(*[
            tcp_raw_client.send(tcp_raw_client.valid_syn_packet)
            for _ in range(10)
        ])
...
async def test_multiple_requests_logs_without_blocking(
    ...
    results = await asyncio.gather(*[
        check_connection(client, udp_ip4_server)
        for client in clients
    ])
